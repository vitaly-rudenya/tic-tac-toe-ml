...
class Agent:
    ...
    def check_game_compatibility(self, game):
        ...
        if len(game_output_shape) != len(self.model.get_input_shape_at(0)):
            ...
            for i in range(len(self.model.get_input_shape_at(0))):
                if self.model.get_input_shape_at(0)[i] and game_output_shape[i] and self.model.get_input_shape_at(0)[
                    i] != game_output_shape[i]:
                    raise Exception('Dimension mismatch. Input shape of the model should be compatible with the game.')
    ...
    def train(self, game, nb_epoch=1000, batch_size=50, gamma=0.9, epsilon=None, epsilon_rate=0.5,
        ...
        for epoch in range(nb_epoch):
            ...
            while not game_over_flag:
                if np.random.random() < epsilon or epoch < observe:
                    ...
                    q = model.predict(game_data)
                    action = int(np.argmax(q[0]))
                game.play(action)
                reward = game.get_score()
                reward_sum += reward
                S_prime = self.get_game_data(game)
                game_over_flag = game.is_over()
                transition = [game_data, action, reward, S_prime, game_over_flag]
            ...
            if epsilon > final_epsilon and epoch >= observe:
                epsilon -= delta
            print("Epoch {:03d}/{:03d} | Loss {:.4f} | Epsilon {:.2f} | Reward: {:.1f} | Win count {}".format(epoch + 1,
                                                                                                              nb_epoch,
                                                                                                              loss,
                                                                                                              epsilon,
                                                                                                              reward_sum,
